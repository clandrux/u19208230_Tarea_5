...
function CurrentProjectID() {
	...
	return "{B36B93AF-B58F-461B-B767-5F08C12493E9}";
}
...
class cExportBase {
    ...
    function ExportText($txt) {
            switch ($this->Table->Export) {
                ...
                if ($this->Line <> "")
                    $this->Line .= ",";
                    $this->Line .= "\"" . str_replace("\"", "\"\"", strval($txt)) . "\"";
                    break;
            }
    }
	...
	function ExportHeaderAndFooter() {
		...
		// Begin of modification Add Record Number Column on Exported List, modified by Masino Sinaga, June 3, 2012
        global $Language;
        $header = "<html><head><title>".$Language->ProjectPhrase("BodyTitle")."</title>\r\n";
...
// Begin of modification Add Record Number Column on Exported List, modified by Masino Sinaga, June 3, 2012
class cExportPrint extends cExportBase {

    function ExportHeaderAndFooter() {
        global $Language;
        $header = "<html><head><title>".$Language->ProjectPhrase("BodyTitle")."</title>\r\n";
        $header .= $this->CharsetMetaTag();
        $header .= "<style type=\"text/css\">.table-bi { background: #fff; border-collapse: collapse; margin-bottom:7px; font-family:tahoma;font-size:8pt; } .table-bi td { border: 1px solid #bbb; background: #fff; padding: 5px; } .table-bi th { border: 1px solid #aaa; text-align: left; background: #F7F7F7; font-weight: bold; padding: 6px; }</style>\r\n";
        $header .= "</" . "head>\r\n<body>\r\n";
        $this->Text = $header . $this->Text . "</body></html>";
    }  
...
class cNumericPager {
	...
	var $FirstButton, $PrevButton, $NextButton, $LastButton, $CurrentPage, $PageCount; // added $CurrentPage and $PageCount to hide error undefined, by Masino Sinaga, July 19, 2014
	var $ButtonCount = 0;
	var $Visible = TRUE;
	...
	function __construct($StartRec, $DisplayRecs, $TotalRecs, $RecRange)
	{
		...
		// Begin of modification of Overcome Error undefined CurrentPage or PageCount, by Masino Sinaga, July 19, 2014
		$this->CurrentPage = intval(($this->FromIndex-1)/$this->PageSize) + 1;
		$this->PageCount = intval(($this->RecordCount-1)/$this->PageSize) + 1;

		// End of modification of Overcome Error undefined CurrentPage or PageCount, by Masino Sinaga, July 19, 2014
...
function MasinoBreadcrumbLinks() {
	global $Language, $UserTableConn;
	$bcl = "";
	$nav = "<ul class=\"breadcrumb\">";
	$sSql = "SELECT C.*
			FROM ".MS_MASINO_BREADCRUMBLINKS_TABLE." AS B, ".MS_MASINO_BREADCRUMBLINKS_TABLE." AS C
			WHERE (B.Lft BETWEEN C.Lft AND C.Rgt) AND (B.Page_URL LIKE '".ew_CurrentPage()."')
			ORDER BY C.Lft";
	...
	while (!$rs->EOF) {
		if ($i < $recCount) {
			if ($i==1) {
				$nav .= "<li><a href='index.php' title='".$Language->BreadcrumbPhrase("Home")."' alt='".$Language->BreadcrumbPhrase("Home")."' class='ewHome'><span class='glyphicon glyphicon-home ewIconHome'></span></a><span class=\"divider\">".MS_BREADCRUMBLINKS_DIVIDER."</span></li>";
			} else {
	...
	if (MS_SHOW_HELP_ONLINE) {
		$nav .= "&nbsp;<a href='javascript:void(0);' id='helponline' onclick='msHelpDialogShow()' title='".$Language->Phrase("Help")."'><span class='glyphicon glyphicon-question-sign ewIconHelp'></span></a>";
	}
	$nav .= "</ul>";
	return $nav;
}
...
class cField {
	...
	// No need this anymore! Must be removed in order to implement overriding constant in Table level, by Masino Sinaga, July 22, 2014
	...
	function ExportValue() {
		...
		// We will use the same way with ExportCaption method above, by Masino Sinaga, July 22, 2014

		return (EW_EXPORT_ORIGINAL_VALUE) ? $this->CurrentValue : $this->ViewValue;
	}
...
class cListOptions {
	...
	function RenderDropDownButton($body, $pos) {
		...
		if ($links <> "") {
			...
			$buttonclass = "dropdown-toggle btn btn-default btn-sm"; // added btn-sm by Masino Sinaga, December 20, 2014 in order to make its size same with search button
			if ($this->ButtonClass <> "")
...
class cAdvancedSecurity {
    ...
    // Can export to Print
    function CanExportToPrint() {
        return (($this->CurrentUserLevel & MS_ALLOW_EXPORT_TO_PRINT) == MS_ALLOW_EXPORT_TO_PRINT);
    }

    function setCanExportToPrint($b) {
        if ($b) {
            $this->CurrentUserLevel = ($this->CurrentUserLevel | MS_ALLOW_EXPORT_TO_PRINT);
        } else {
            $this->CurrentUserLevel = ($this->CurrentUserLevel & (~ MS_ALLOW_EXPORT_TO_PRINT));
        }
    }
    ...
    // Can export to HTML
    function CanExportToHTML() {
        return (($this->CurrentUserLevel & MS_ALLOW_EXPORT_TO_HTML) == MS_ALLOW_EXPORT_TO_HTML);
    }

    function setCanExportToHTML($b) {
        if ($b) {
            $this->CurrentUserLevel = ($this->CurrentUserLevel | MS_ALLOW_EXPORT_TO_HTML);
        } else {
            $this->CurrentUserLevel = ($this->CurrentUserLevel & (~ MS_ALLOW_EXPORT_TO_HTML));
        }
    } 
    ...
    // Can export to Excel
    function CanExportToExcel() {
        return (($this->CurrentUserLevel & MS_ALLOW_EXPORT_TO_EXCEL) == MS_ALLOW_EXPORT_TO_EXCEL);
    }

    function setCanExportToExcel($b) {
        if ($b) {
            $this->CurrentUserLevel = ($this->CurrentUserLevel | MS_ALLOW_EXPORT_TO_EXCEL);
        } else {
            $this->CurrentUserLevel = ($this->CurrentUserLevel & (~ MS_ALLOW_EXPORT_TO_EXCEL));
        }
    }
    ...
    // Can export to Word
    function CanExportToWord() {
        return (($this->CurrentUserLevel & MS_ALLOW_EXPORT_TO_WORD) == MS_ALLOW_EXPORT_TO_WORD);
    }

    function setCanExportToWord($b) {
        if ($b) {
            $this->CurrentUserLevel = ($this->CurrentUserLevel | MS_ALLOW_EXPORT_TO_WORD);
        } else {
            $this->CurrentUserLevel = ($this->CurrentUserLevel & (~ MS_ALLOW_EXPORT_TO_WORD));
        }
    }     
    ...
    // Can export to PDF
    function CanExportToPDF() {
        return (($this->CurrentUserLevel & MS_ALLOW_EXPORT_TO_PDF) == MS_ALLOW_EXPORT_TO_PDF);
    }

    function setCanExportToPDF($b) {
        if ($b) {
            $this->CurrentUserLevel = ($this->CurrentUserLevel | MS_ALLOW_EXPORT_TO_PDF);
        } else {
            $this->CurrentUserLevel = ($this->CurrentUserLevel & (~ MS_ALLOW_EXPORT_TO_PDF));
        }
    }     
    ...
    // Can export to XML
    function CanExportToXML() {
        return (($this->CurrentUserLevel & MS_ALLOW_EXPORT_TO_XML) == MS_ALLOW_EXPORT_TO_XML);
    }

    function setCanExportToXML($b) {
        if ($b) {
            $this->CurrentUserLevel = ($this->CurrentUserLevel | MS_ALLOW_EXPORT_TO_XML);
        } else {
            $this->CurrentUserLevel = ($this->CurrentUserLevel & (~ MS_ALLOW_EXPORT_TO_XML));
        }
    }     
    ...
    function CanExportToCSV() {
        return (($this->CurrentUserLevel & MS_ALLOW_EXPORT_TO_CSV) == MS_ALLOW_EXPORT_TO_CSV);
    }

    function setCanExportToCSV($b) {
        if ($b) {
            $this->CurrentUserLevel = ($this->CurrentUserLevel | MS_ALLOW_EXPORT_TO_CSV);
        } else {
            $this->CurrentUserLevel = ($this->CurrentUserLevel & (~ MS_ALLOW_EXPORT_TO_CSV));
        }
    }     
    ...
    // Can export to Email
    function CanExportToEmail() {
        return (($this->CurrentUserLevel & MS_ALLOW_EXPORT_TO_EMAIL) == MS_ALLOW_EXPORT_TO_EMAIL);
    }

    function setCanExportToEmail($b) {
        if ($b) {
            $this->CurrentUserLevel = ($this->CurrentUserLevel | MS_ALLOW_EXPORT_TO_EMAIL);
        } else {
            $this->CurrentUserLevel = ($this->CurrentUserLevel & (~ MS_ALLOW_EXPORT_TO_EMAIL));
        }
    }     
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					...
					if ($ValidateUser) {
						...
						} else { // this is EW_USER_PROFILE_CONCURRENT_SESSION_COUNT > 1 (>= v11)
							if ($UserProfile->IsValidUser($usr, session_id())) {
							} else {
...
class cUserProfile {
	...
	function __construct() {
		...
		$this->Profile[@MS_USER_PROFILE_REGISTERED_DATE_TIME] = "";
		$this->Profile[@MS_USER_PROFILE_LAST_LOGIN_DATE_TIME] = "";
		$this->Profile[@MS_USER_PROFILE_LAST_LOGOUT_DATE_TIME] = "";
