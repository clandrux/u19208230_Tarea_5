...
$a_sales_preview = NULL; // Initialize page object first

class ca_sales_preview extends ca_sales {
	...
	// Page object name
	var $PageObjName = 'a_sales_preview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'a_sales');
	...
	function MasterKeyUrl() {
		$mastertblvar = @$_GET["t"];
		$url = "";
		if ($mastertblvar == "a_customers") {
			$url = "" . EW_TABLE_SHOW_MASTER . "=a_customers&fk_Customer_Number=" . urlencode(strval($this->Customer_ID->QueryStringValue)) . "";
		}
		return $url;
	}
		...
		if ($mastertblvar == "a_customers") {
			...
			if ($x !== FALSE) {
				...
				$val = $this->UnquoteValue($val, "DB");
 				$this->Customer_ID->setQueryStringValue($val);
			}
		}
	}
...
// Create page object
if (!isset($a_sales_preview)) $a_sales_preview = new ca_sales_preview();

// Page init
$a_sales_preview->Page_Init();

// Page main
$a_sales_preview->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$a_sales_preview->Page_Render();
?>
<?php $a_sales_preview->ShowPageHeader(); ?>
<?php if ($a_sales_preview->TotalRecs > 0) { ?>
<div class="ewGrid">
...
// Render list options
$a_sales_preview->RenderListOptions();
...
<?php if ($a_sales->Sales_Number->Visible) { // Sales_Number ?>
			<th><?php echo $a_sales->Sales_Number->FldCaption() ?></th>
<?php } ?>
<?php if ($a_sales->Sales_Date->Visible) { // Sales_Date ?>
			<th><?php echo $a_sales->Sales_Date->FldCaption() ?></th>
<?php } ?>
<?php if ($a_sales->Customer_ID->Visible) { // Customer_ID ?>
			<th><?php echo $a_sales->Customer_ID->FldCaption() ?></th>
<?php } ?>
<?php if ($a_sales->Total_Amount->Visible) { // Total_Amount ?>
			<th style="white-space: nowrap;"><?php echo $a_sales->Total_Amount->FldCaption() ?></th>
<?php } ?>
<?php if ($a_sales->Discount_Amount->Visible) { // Discount_Amount ?>
			<th style="white-space: nowrap;"><?php echo $a_sales->Discount_Amount->FldCaption() ?></th>
<?php } ?>
<?php if ($a_sales->Tax_Amount->Visible) { // Tax_Amount ?>
			<th style="white-space: nowrap;"><?php echo $a_sales->Tax_Amount->FldCaption() ?></th>
<?php } ?>
<?php if ($a_sales->Final_Total_Amount->Visible) { // Final_Total_Amount ?>
			<th style="white-space: nowrap;"><?php echo $a_sales->Final_Total_Amount->FldCaption() ?></th>
<?php } ?>
<?php if ($a_sales->Total_Payment->Visible) { // Total_Payment ?>
			<th style="white-space: nowrap;"><?php echo $a_sales->Total_Payment->FldCaption() ?></th>
<?php } ?>
<?php if ($a_sales->Total_Balance->Visible) { // Total_Balance ?>
			<th style="white-space: nowrap;"><?php echo $a_sales->Total_Balance->FldCaption() ?></th>
<?php } ?>
<?php

// Render list options (header, right)
$a_sales_preview->ListOptions->Render("header", "right");
?>
		</tr>
	</thead>
	<tbody><!-- Table body -->
<?php
$a_sales_preview->RecCount = 0;
$a_sales_preview->RowCnt = 0;
$rowNumber = 0;
while ($a_sales_preview->Recordset && !$a_sales_preview->Recordset->EOF) {
	$rowNumber++;

	// Init row class and style
	$a_sales_preview->RecCount++;
	$a_sales_preview->RowCnt++;
	$a_sales_preview->CssStyle = "";
	$a_sales_preview->LoadListRowValues($a_sales_preview->Recordset);
	$a_sales_preview->AggregateListRowValues(); // Aggregate row values

	// Render row
	$a_sales_preview->RowType = EW_ROWTYPE_PREVIEW; // Preview record
	$a_sales_preview->RenderListRow();
...
$a_sales_preview->ListOptions->Render("body", "right", $a_sales_preview->RowCnt);
?>
	</tr>
<?php
	$a_sales_preview->Recordset->MoveNext();
}
?>
	</tbody>
<?php
	...
	$a_sales_preview->AggregateListRow(); // Prepare aggregate row
...
$a_sales_preview->ListOptions->Render("footer", "left");
?>
<?php if ($a_sales->Sales_Number->Visible) { // Sales_Number ?>
		<!-- Sales_Number -->
		<td>
...
<?php echo $a_sales->Total_Amount->ViewValue ?>
		</td>
<?php } ?>
<?php if ($a_sales->Discount_Amount->Visible) { // Discount_Amount ?>
		<!-- Discount_Amount -->
		<td>
<span class="ewAggregate"><?php echo $Language->Phrase("TOTAL") ?></span>
<?php echo $a_sales->Discount_Amount->ViewValue ?>
		</td>
<?php } ?>
<?php if ($a_sales->Tax_Amount->Visible) { // Tax_Amount ?>
		<!-- Tax_Amount -->
		<td>
<span class="ewAggregate"><?php echo $Language->Phrase("TOTAL") ?></span>
<?php echo $a_sales->Tax_Amount->ViewValue ?>
		</td>
<?php } ?>
<?php if ($a_sales->Final_Total_Amount->Visible) { // Final_Total_Amount ?>
		<!-- Final_Total_Amount -->
		<td>
<span class="ewAggregate"><?php echo $Language->Phrase("TOTAL") ?></span>
<?php echo $a_sales->Final_Total_Amount->ViewValue ?>
		</td>
<?php } ?>
<?php if ($a_sales->Total_Payment->Visible) { // Total_Payment ?>
		<!-- Total_Payment -->
		<td>
<span class="ewAggregate"><?php echo $Language->Phrase("TOTAL") ?></span>
<?php echo $a_sales->Total_Payment->ViewValue ?>
		</td>
<?php } ?>
<?php if ($a_sales->Total_Balance->Visible) { // Total_Balance ?>
		<!-- Total_Balance -->
		<td>
<span class="ewAggregate"><?php echo $Language->Phrase("TOTAL") ?></span>
<?php echo $a_sales->Total_Balance->ViewValue ?>
		</td>
<?php } ?>
<?php

// Render list options (footer, right)
$a_sales_preview->ListOptions->Render("footer", "right");
?>
	</tr>
	</tfoot>
</table>
...
<?php if ($a_sales_preview->TotalRecs > 0) { ?>
<div class="ewDetailCount"><?php echo $a_sales_preview->TotalRecs ?>&nbsp;<?php echo $Language->Phrase("Record") ?></div>
<?php // } else { ?>
<?php // } ?>
<div class="ewPreviewOtherOptions">
<?php
	foreach ($a_sales_preview->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
...
<?php
$a_sales_preview->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php
if ($a_sales_preview->Recordset)
	$a_sales_preview->Recordset->Close();
?>
<?php } else { // if ($a_sales_preview->TotalRecs > 0)  ?>
...
} // end if ($a_sales_preview->TotalRecs > 0)
...
?>
<?php
$a_sales_preview->Page_Terminate();
?>
