...
$a_sales_detail_preview = NULL; // Initialize page object first

class ca_sales_detail_preview extends ca_sales_detail {
	...
	// Page object name
	var $PageObjName = 'a_sales_detail_preview';
	...
	function __construct() {
		...
		if (!defined("EW_TABLE_NAME"))
			define("EW_TABLE_NAME", 'a_sales_detail');
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'a_sales_detail');
	...
	function MasterKeyUrl() {
		$mastertblvar = @$_GET["t"];
		$url = "";
		if ($mastertblvar == "a_sales") {
			$url = "" . EW_TABLE_SHOW_MASTER . "=a_sales&fk_Sales_Number=" . urlencode(strval($this->Sales_Number->QueryStringValue)) . "";
		}
		return $url;
	}
		...
		if ($mastertblvar == "a_sales") {
			...
			if ($x !== FALSE) {
				...
				$val = $this->UnquoteValue($val, "DB");
 				$this->Sales_Number->setQueryStringValue($val);
			}
		}
	}
...
// Create page object
if (!isset($a_sales_detail_preview)) $a_sales_detail_preview = new ca_sales_detail_preview();

// Page init
$a_sales_detail_preview->Page_Init();

// Page main
$a_sales_detail_preview->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$a_sales_detail_preview->Page_Render();
?>
<?php $a_sales_detail_preview->ShowPageHeader(); ?>
<?php if ($a_sales_detail_preview->TotalRecs > 0) { ?>
<div class="ewGrid">
...
// Render list options
$a_sales_detail_preview->RenderListOptions();

// Render list options (header, left)
$a_sales_detail_preview->ListOptions->Render("header", "left");
?>
		<?php 		
...
<?php if ($a_sales_detail->Supplier_Number->Visible) { // Supplier_Number ?>
			<th><?php echo $a_sales_detail->Supplier_Number->FldCaption() ?></th>
<?php } ?>
<?php if ($a_sales_detail->Stock_Item->Visible) { // Stock_Item ?>
			<th><?php echo $a_sales_detail->Stock_Item->FldCaption() ?></th>
<?php } ?>
<?php if ($a_sales_detail->Sales_Quantity->Visible) { // Sales_Quantity ?>
			<th><?php echo $a_sales_detail->Sales_Quantity->FldCaption() ?></th>
<?php } ?>
<?php if ($a_sales_detail->Purchasing_Price->Visible) { // Purchasing_Price ?>
			<th><?php echo $a_sales_detail->Purchasing_Price->FldCaption() ?></th>
<?php } ?>
<?php if ($a_sales_detail->Sales_Price->Visible) { // Sales_Price ?>
			<th><?php echo $a_sales_detail->Sales_Price->FldCaption() ?></th>
<?php } ?>
<?php if ($a_sales_detail->Sales_Total_Amount->Visible) { // Sales_Total_Amount ?>
			<th><?php echo $a_sales_detail->Sales_Total_Amount->FldCaption() ?></th>
<?php } ?>
<?php

// Render list options (header, right)
$a_sales_detail_preview->ListOptions->Render("header", "right");
?>
		</tr>
	</thead>
	<tbody><!-- Table body -->
<?php
$a_sales_detail_preview->RecCount = 0;
$a_sales_detail_preview->RowCnt = 0;
$rowNumber = 0;
while ($a_sales_detail_preview->Recordset && !$a_sales_detail_preview->Recordset->EOF) {
	$rowNumber++;

	// Init row class and style
	$a_sales_detail_preview->RecCount++;
	$a_sales_detail_preview->RowCnt++;
	$a_sales_detail_preview->CssStyle = "";
	$a_sales_detail_preview->LoadListRowValues($a_sales_detail_preview->Recordset);
	$a_sales_detail_preview->AggregateListRowValues(); // Aggregate row values

	// Render row
	$a_sales_detail_preview->RowType = EW_ROWTYPE_PREVIEW; // Preview record
	$a_sales_detail_preview->RenderListRow();

	// Render list options
	$a_sales_detail_preview->RenderListOptions();
?>
	<tr<?php echo $a_sales_detail_preview->RowAttributes() ?>>
<?php

// Render list options (body, left)
$a_sales_detail_preview->ListOptions->Render("body", "left", $a_sales_detail_preview->RowCnt);
?>
...
<?php if ($a_sales_detail->Supplier_Number->Visible) { // Supplier_Number ?>
		<!-- Supplier_Number -->
		<td<?php echo $a_sales_detail->Supplier_Number->CellAttributes() ?>>
...
<?php if ($a_sales_detail->Stock_Item->Visible) { // Stock_Item ?>
		<!-- Stock_Item -->
		<td<?php echo $a_sales_detail->Stock_Item->CellAttributes() ?>>
...
<?php if ($a_sales_detail->Sales_Quantity->Visible) { // Sales_Quantity ?>
		<!-- Sales_Quantity -->
		<td<?php echo $a_sales_detail->Sales_Quantity->CellAttributes() ?>>
...
<?php if ($a_sales_detail->Purchasing_Price->Visible) { // Purchasing_Price ?>
		<!-- Purchasing_Price -->
		<td<?php echo $a_sales_detail->Purchasing_Price->CellAttributes() ?>>
...
<?php if ($a_sales_detail->Sales_Price->Visible) { // Sales_Price ?>
		<!-- Sales_Price -->
		<td<?php echo $a_sales_detail->Sales_Price->CellAttributes() ?>>
...
<?php if ($a_sales_detail->Sales_Total_Amount->Visible) { // Sales_Total_Amount ?>
		<!-- Sales_Total_Amount -->
		<td<?php echo $a_sales_detail->Sales_Total_Amount->CellAttributes() ?>>
...
$a_sales_detail_preview->ListOptions->Render("body", "right", $a_sales_detail_preview->RowCnt);
?>
	</tr>
<?php
	$a_sales_detail_preview->Recordset->MoveNext();
}
?>
	</tbody>
<?php
	...
	$a_sales_detail_preview->AggregateListRow(); // Prepare aggregate row
...
$a_sales_detail_preview->ListOptions->Render("footer", "left");
?>
<?php if ($a_sales_detail->Supplier_Number->Visible) { // Supplier_Number ?>
		<!-- Supplier_Number -->
		<td>
...
$a_sales_detail_preview->ListOptions->Render("footer", "right");
?>
	</tr>
	</tfoot>
</table>
...
<?php if ($a_sales_detail_preview->TotalRecs > 0) { ?>
<div class="ewDetailCount"><?php echo $a_sales_detail_preview->TotalRecs ?>&nbsp;<?php echo $Language->Phrase("Record") ?></div>
<?php // } else { ?>
<?php // } ?>
<div class="ewPreviewOtherOptions">
<?php
	foreach ($a_sales_detail_preview->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
...
<?php
$a_sales_detail_preview->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php
if ($a_sales_detail_preview->Recordset)
	$a_sales_detail_preview->Recordset->Close();
?>
<?php } else { // if ($a_sales_detail_preview->TotalRecs > 0)  ?>
...
} // end if ($a_sales_detail_preview->TotalRecs > 0)
...
?>
<?php
$a_sales_detail_preview->Page_Terminate();
?>
